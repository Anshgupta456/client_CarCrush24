          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-start">
              ...
              <article className="lg:col-span-8 flex flex-col">
                ...
                {/* Rendered Sections or Rich Content */}
                <div className="space-y-10 sm:space-y-12">
                  {blog.contentHtml ? (
                    <div
                      className="blog-rich-content text-sm sm:text-[15.5px] text-[#374151] leading-relaxed space-y-5"
                    ...
                    blog.sections.map((sec) => (
                      <div key={sec.id} id={sec.id} className="scroll-mt-32">
                        ...
                        {sec.callout && (
                          <div
                            className={`mt-5 p-5 rounded-2xl border flex items-start gap-3.5 ${sec.callout.type === 'warning'
                              ...
                              {sec.gallery.map((imgItem, gIdx) => (
                                <figure key={gIdx} className="flex flex-col">
                                  <div className="relative w-full h-48 sm:h-56 rounded-2xl overflow-hidden shadow-xs border border-[#E5EAE5] bg-gray-100 group">
                                    <Image
                                      src={imgItem.url}
                                      alt={imgItem.caption || `Gallery image ${gIdx + 1}`}
                                      fill
                ...
                {/* Top-Level Gallery (For Rich Text & Admin Uploaded Gallery) */}
                {blog.gallery && blog.gallery.length > 0 && (
                  <div className="mt-12 pt-8 border-t border-[#E5EAE5]">
                    <h3 className="text-xl sm:text-2xl font-black text-[#111827] tracking-tight mb-6 flex items-center gap-3">
                      <span className="w-2.5 h-6 bg-[#6FCF3C] rounded-full flex-shrink-0" />
                      <span>Photo &amp; Document Gallery</span>
                    </h3>
                    <div className={`grid grid-cols-1 ${blog.gallery.length > 1 ? 'sm:grid-cols-2' : ''} gap-5 sm:gap-6`}>
                      {blog.gallery.map((imgItem, gIdx) => (
                        <figure key={gIdx} className="flex flex-col">
                          <div className="relative w-full h-52 sm:h-64 rounded-2xl overflow-hidden shadow-xs border border-[#E5EAE5] bg-gray-100 group">
                            <Image
                              src={imgItem.url}
                              alt={imgItem.caption || `Gallery photo ${gIdx + 1}`}
                              fill
                              className="object-cover group-hover:scale-105 transition-transform duration-500"
                            />
                          </div>
