...
export default function TermsPage() {
  ...
  const [policy, setPolicy] = useState(defaultPolicies.terms);
  const [activeSection, setActiveSection] = useState('');

  // Fetch updated terms from API
  useEffect(() => {
    let ignore = false;

    const fetchPolicy = async () => {
      try {
        const res = await fetch('/api/policies/terms');
        if (res.ok && !ignore) {
        ...
        console.warn('[Terms] Fetch failed, using default template:', err.message);
      }
    };
  ...
  const sections = policy?.sections || defaultPolicies.terms.sections;
    ...
    <div className="min-h-screen bg-[#F8F9F5] text-[#131A15] flex flex-col selection:bg-[#6FCF3C]/30 selection:text-[#16311F]">
      ...
      <section className="relative pt-32 pb-14 sm:pt-36 sm:pb-20 bg-gradient-to-b from-[#112317] via-[#0E1C12] to-[#0A150D] text-white overflow-hidden border-b border-[#1C3621]">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#188A38]/20 border border-[#6FCF3C]/30 text-[#6FCF3C] text-xs font-semibold tracking-wide uppercase shadow-2xs">
            ...
            <span>Statutory Scrappage Agreement • MoRTH RVSF Terms</span>
          </div>
          ...
          <p className="text-sm sm:text-base text-[#A1B2A5] max-w-2xl mx-auto leading-relaxed">
            {policy?.summary ||
              'Standard contractual terms, vehicle ownership eligibility, pricing guidelines, and statutory depollution conditions.'}
          </p>

          {/* Meta Ribbon */}
          <div className="pt-3 flex flex-wrap items-center justify-center gap-3 sm:gap-6 text-xs text-[#C5D4C9]">
            <div className="flex items-center gap-1.5 bg-white/5 px-3 py-1.5 rounded-full border border-white/10">
              <FileText className="w-3.5 h-3.5 text-[#6FCF3C]" />
              <span>
            ...
            <div className="flex items-center gap-1.5 bg-white/5 px-3 py-1.5 rounded-full border border-white/10">
              <ShieldCheck className="w-3.5 h-3.5 text-[#6FCF3C]" />
              <span>RVSF Rules 2021 Enforceable</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          {/* Left Column: Quick Clause Navigator */}
          <aside className="lg:col-span-4 order-2 lg:order-1">
            <div className="sticky top-28 space-y-6">
              ...
              {/* Legal Notice */}
              <div className="bg-amber-50 border border-amber-200 rounded-3xl p-6 shadow-2xs space-y-2.5">
                <div className="flex items-center gap-2 text-amber-800 font-bold text-xs">
                  <AlertTriangle className="w-4 h-4 text-amber-700 flex-shrink-0" />
                  <span>Important Compliance Note</span>
                </div>
                <p className="text-[11.5px] text-amber-900/80 leading-relaxed">
                  Vehicles with bank hypothecation must provide a valid Form 35 loan foreclosure NOC. Tampered chassis or disputed ownership cannot be accepted under MoRTH rules.
                </p>
              </div>

              {/* Authorized Entity Coordinates */}
          ...
          {/* Right Column: Terms Document Clauses */}
            ...
            <div className="p-6 rounded-2xl bg-[#F8F9F5] border border-[#E4E7DE] space-y-3 mt-8">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs font-extrabold uppercase tracking-wider text-[#131A15]">
                  Questions Regarding Scrappage Agreements?
                </h4>
              </div>
              <p className="text-xs text-[#5B6660] leading-relaxed">
                Contact our authorized compliance team at{' '}
                <a
